...
// http://mathworld.wolfram.com/Quaternion.html
float3 q_rotate_vector(float3 v, float4 r)
{
...
float4 q_rotate_angle_axis(float angle, float3 axis)
{
...
float4 q_from_to_rotation(float3 v1, float3 v2)
{
	float4 q;
	float d = dot(v1, v2);
	if (d < -0.999999)
...
float4x4 q_to_matrix(float4 quat)
{

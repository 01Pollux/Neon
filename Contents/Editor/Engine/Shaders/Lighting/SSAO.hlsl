	...
	uint3 DTID : SV_DispatchThreadID)
{
	...
	float3 Normal = normalize(NormalMap.SampleLevel(s_Sampler_PointClamp, Uv, 0.f).xyz * 2.f - 1.f);
	float3 Random = normalize(NoiseMap.SampleLevel(s_Sampler_LinearWrap, NoiseUv * NoiseScale, 0.f).xyz * 2.f - 1.f);
	
	float Depth = DepthMap.SampleLevel(s_Sampler_PointClamp, Uv, 0.f).x;
	...
	for (int i = 0; i < SSAO_SAMPLE_COUNT; ++i)
	{
		...
		float SampleDepth = DepthMap.SampleLevel(s_Sampler_PointClamp, Offset.xy, 0.f).x;
	...
	Occlusion = pow(1.f - Occlusion, c_SSAOParams.Magnitude);

...
namespace efsw
{
    ...
    DirWatcherGeneric::DirWatcherGeneric(DirWatcherGeneric* parent, WatcherGeneric* ws,
                                         ...
                                         bool reportNewFiles) :
        Parent(parent),
        Watch(ws), Recursive(recursive), Deleted(false)
    {
    ...
    void DirWatcherGeneric::resetDirectory(std::string directory)
    {
        ...
        if (Watch->Directory != directory)
        {
                   ...
                   directory.at(directory.size() - 1) == FileSystem::getOSSlash())))
            {
                ...
                NEON_VALIDATE(Parent != NULL, "Direocy parent is NULL. Fatal error.");
    ...
    DirWatcherGeneric* DirWatcherGeneric::findDirWatcherFast(std::string dir)
    {
        ...
        NEON_ASSERT(!dir.empty());
        NEON_ASSERT(dir.size() >= DirSnap.DirectoryInfo.Filepath.size());
        NEON_ASSERT(DirSnap.DirectoryInfo.Filepath ==
        ...
        if (dir.size() == 1)
        {
            NEON_ASSERT(dir[0] == FileSystem::getOSSlash());
            return this;
        }

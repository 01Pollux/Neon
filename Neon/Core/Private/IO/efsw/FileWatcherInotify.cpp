...
#include <Private/IO/efsw/inotify-nosys.h>
#else
#include <sys/inotify.h>
#endif

#include <Private/IO/efsw/FileSystem.hpp>
#include <Private/IO/efsw/Lock.hpp>
...
namespace efsw
{
        ...
        FileWatcherImpl(parent), mFD(-1), mThread(NULL), mIsTakingAction(false)
    {
        ...
        NEON_VALIDATE(mFD >= 0, "inotify_init failed {}", strerror(errno));
        mInitOK = true;
    }
                                         ...
                                         bool recursive, WatcherInotify* parent)
    {
        ...
        WatcherInotify* pWatch = NEON_NEW WatcherInotify();
    ...
    void FileWatcherInotify::watch()
    {
        if (NULL == mThread)
        {
            mThread = NEON_NEW Thread(&FileWatcherInotify::run, this);
            mThread->launch();
        }
    }
    ...
    {
        char* buff = NEON_NEW char[BUFF_SIZE];
    ...
    void FileWatcherInotify::checkForNEON_NEWWatcher(Watcher* watch, std::string fpath)
    {
        FileSystem::dirAddSlashAtEnd(fpath);

        /// If the watcher is recursive, checks if the NEON_NEW file is a folder, and creates a watcher
                                          ...
                                          unsigned long action, std::string)
    {
        ...
        else if (IN_MOVED_TO & action)
        {
            ...
            if (watch->OldFileName.empty())
            {
                ...
                checkForNEON_NEWWatcher(watch, fpath);
            }
            else
            {
                watch->Listener->handleFileAction(watch->ID, watch->Directory, filename,
            ...
            if (watch->Recursive && FileSystem::isDirectory(fpath))
            {
                /// Update the NEON_NEW directory path

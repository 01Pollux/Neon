...
namespace Neon::Asio
{
    ...
    template<typename FunctionType = Impl::default_function_type,
    ...
    public:
        ...
        /// thread pool is movable
        ThreadPool(ThreadPool&&)            = default;
        ThreadPool& operator=(ThreadPool&&) = default;

        /**
         * @brief Enqueue a task into the thread pool that returns a result.
         * @Impl Note that task execution begins once the task is enqueued.
    ...
    private:
        ...
        struct task_item
        {
            QueueTS<FunctionType> tasks{};
            std::binary_semaphore signal{ 0 };
        };

        std::vector<ThreadType>  threads_;
        std::deque<task_item>    tasks_;
        QueueTS<std::size_t>     priority_queue_;

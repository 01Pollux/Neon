...
namespace Renderer
{
        ...
        const MaterialMetaDataBuilder& Builder)
    {
        ...
        for (auto& Entry : Builder.m_Entries)
        {
            ...
            if (Entry.m_PreferDescriptor)
            {
                ...
                RootSigDesc.AddDescriptorTable(Table, Entry.m_Visiblity);
            ...
            else
            {
                ...
                case MaterialDataType::Constants:
                {
                    ...
                    RootSigDesc.Add32BitConstants(Register, Space, uint32_t(DataSize / sizeof(int)), Entry.m_Visiblity);
                    break;
                }
                case MaterialDataType::ConstantBuffer:
                {
                    auto& Root    = ResourceData.Entry.emplace<RootEntry>();
                    Root.ViewType = RHI::ResourceViewType::Cbv;
                    Root.Resources.resize(Entry.m_ArraySize);
                    for (uint16_t i = 0; i < Entry.m_ArraySize; i++)
                    {
                        RootIndex++;
                        RootSigDesc.AddConstantBufferView(Register + i, Space, D3D12_ROOT_DESCRIPTOR_FLAG_NONE, Entry.m_Visiblity);
                    }
                    break;
                }
                case MaterialDataType::Resource:
                {
                    auto& Root    = ResourceData.Entry.emplace<RootEntry>();
                    Root.ViewType = RHI::ResourceViewType::Srv;
                    Root.Resources.resize(Entry.m_ArraySize);
                    for (uint16_t i = 0; i < Entry.m_ArraySize; i++)
                    {
                        RootIndex++;
                        RootSigDesc.AddShaderResourceView(Register + i, Space, D3D12_ROOT_DESCRIPTOR_FLAG_NONE, Entry.m_Visiblity);
                    }
                    break;
                }
                case MaterialDataType::UavResource:
                {
                    auto& Root    = ResourceData.Entry.emplace<RootEntry>();
                    Root.ViewType = RHI::ResourceViewType::Uav;
                    Root.Resources.resize(Entry.m_ArraySize);
                    for (uint16_t i = 0; i < Entry.m_ArraySize; i++)
                    {
                        RootIndex++;
                        RootSigDesc.AddUnorderedAccessView(Register + i, Space, D3D12_ROOT_DESCRIPTOR_FLAG_NONE, Entry.m_Visiblity);
                    }
                    break;
                }
                case MaterialDataType::Sampler:
                {
                    ...
                    {
                        RootSigDesc.AddSampler(Sampler.BuildStatic(RegOffset++, Space, Entry.m_Visiblity));
                    }
                    break;
                }
                default:
        ...
        m_RootSignature = RHI::RootSignature(RootSigDesc);
        AllocateDescriptors();
    }
    ...
    {
        auto Display = RHI::IRenderDevice::Get()->GetDisplay();
        if (m_RootSignature.GetResourceCount())
        {
            auto DescriptorAllocator = Display->GetStaticDescriptorHeapManager(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
            m_ResourceDescriptor     = DescriptorAllocator->Allocate(m_RootSignature.GetResourceCount());
        }

        if (m_RootSignature.GetSamplerCount())
        {
            auto DescriptorAllocator = Display->GetStaticDescriptorHeapManager(D3D12_DESCRIPTOR_HEAP_TYPE_SAMPLER);
            m_SamplerDescriptor      = DescriptorAllocator->Allocate(m_RootSignature.GetSamplerCount());
        }
    }

    void MaterialMetaData::DestroyDescriptors() const
    {
        auto Display = RHI::IRenderDevice::Get()->GetDisplay();
        if (m_ResourceDescriptor)
        {
            auto DescriptorAllocator = Display->GetStaticDescriptorHeapManager(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
            DescriptorAllocator->Free(m_ResourceDescriptor);
        }

        if (m_SamplerDescriptor)
        {
            auto DescriptorAllocator = Display->GetStaticDescriptorHeapManager(D3D12_DESCRIPTOR_HEAP_TYPE_SAMPLER);
            DescriptorAllocator->Free(m_SamplerDescriptor);
        }
    }
